...
### 階段一：底層資料結構與 Hook 通訊增量重構

- [ ] 擴充 `constants/chart.ts`，加入虛擬列表與成交緩衝區常數。
- [ ] 重構 `useHighFrequencyData.ts` 中的 `TICK` 訊息處理流程，確保主執行緒之歷史緩衝長度可同步擴展至 1000 筆，並封裝高效的局部讀取 API。
- [ ] 實作主執行緒的 `pendingTicksQueue` 暫存邏輯，以支援滾動凍結時的資料雙軌處理。
- [ ] 驗證並確認在 1000 筆的大容量下，雙端執行緒通訊並無因資料傳輸而造成 CPU 佔用率上升。
...
- [ ] 進行效能品質驗證：
  ...
  - 監控 JS Heap 記憶體曲線，確認無階梯狀的記憶體洩漏與密集的 GC 行為。
